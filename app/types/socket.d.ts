...
export interface ServerToClientEvents {
  ...
  'session:initialized': (data: { sessionId: string; userId: number; status: 'online' | 'afk' | 'offline'; restored: boolean }) => void
  // Подтверждение регистрации вкладки
  'tab:registered': (data: { tabId: string; currentPath: string }) => void
}
...
export interface ClientToServerEvents {
  ...
  // ✅ РЕГИСТРАЦИЯ ВКЛАДКИ
  'tab:register': (data: { tabId: string; currentPath?: string }) => void
  // ✅ НАВИГАЦИЯ ПО СТРАНИЦАМ
  'tab:navigate': (data: { currentPath: string }) => void
}
