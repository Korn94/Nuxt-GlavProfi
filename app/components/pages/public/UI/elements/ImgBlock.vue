<template>
  <div class="portfolio-block">
    ...
    <div class="toggle-buttons" v-if="shouldShowToggleButtons">
      <PagesPublicUIButtonsSecondButton v-for="(tab, index) in tabsWithLabels" :key="index" :text="tab.label" :class="{ active: activeTab === tab.key }" :reverseEffect="activeTab !== tab.key" @click="setActiveTab(tab.key)"/>
    </div>
    ...
    <PagesPublicUIButtonsMainButton class="openbtn" text="Показать еще" v-if="filteredImages.length > 9 && !showAll" @click="showAllImages"></PagesPublicUIButtonsMainButton>
  </div>
</template>
