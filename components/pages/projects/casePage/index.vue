<template>
  ...
  <div v-else class="portfolio-case-page">
    ...
    <!-- Хедер кейса -->
    <PagesProjectsCasePageHeader :case-data="caseData" />

    <!-- Основная информация -->
    <PagesProjectsCasePageIntro :case-data="caseData" :works="works" />

    <!-- Сравнение до/после -->
    <div v-if="groupedImages.length > 0" class="comparison-section">
      <PagesProjectsCasePageBeforeAfter
    ...
    <!-- Результаты -->
    <PagesProjectsCasePageResults :case-data="caseData" />

    <!-- Галерея -->
    <PagesProjectsCasePageGallery
...
// Компоненты
import PagesProjectsCasePageHeader from './header.vue'
import PagesProjectsCasePageIntro from './intro.vue'
import PagesProjectsCasePageBeforeAfter from './BeforeAfter.vue'
import PagesProjectsCasePageResults from './results.vue'
import PagesProjectsCasePageGallery from './gallery.vue'
...
onMounted(async () => {
  ...
  // Опционально: вызываем один раз, чтобы установить начальное положение
  handleScroll()
})
</script>
