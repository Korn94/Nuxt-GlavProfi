...
import { handleSubtaskCreate } from '../../../../../socket/handlers/subtasks'
...
export default eventHandler(async (event) => {
  try {
    ...
    // ✅ ОТПРАВЛЯЕМ SOCKET EVENT — ИСПОЛЬЗУЕМ ХЕЛПЕР
    const io = getIO()
    if (io) {
      handleSubtaskCreate(io, newSubtask.id, subtaskForResponse, taskId)
    } else {
      console.log('[API] ⚠️ Socket.IO instance not available')
    }
