// server/api/contractors/me/daily-stats.get.ts
/**
 * Назначение: Получение статистики подневки ТЕКУЩЕГО пользователя-контрагента
 * (только «свои» данные). id/type берутся из авторизованного пользователя,
 * а не из URL — подставить чужого контрагента невозможно.
 *
 * Логика подсчёта (идентична /api/contractors/[type]/[id]/daily-stats):
