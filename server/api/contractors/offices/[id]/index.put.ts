...
export default defineEventHandler(async (event) => {
  ...
  if (!id) throw createError({ statusCode: 400, message: 'ID работника офиса обязателен' })
  ...
  try {
    ...
    const [updatedOffice] = await db.select().from(offices)
      .where(eq(offices.id, parseInt(id)))
    
    if (!updatedOffice) {
      throw createError({ statusCode: 404, message: 'Офисный работник не найден' })
    }

    return updatedOffice
  } catch (error) {
    console.error('Ошибка обновления офисного работника:', error)
    
    // Типизация ошибки
    if (error instanceof Error) {
      throw createError({
        statusCode: 500,
        message: `Ошибка сервера при обновлении офисного работника: ${error.message}`
      })
    } else {
      throw createError({
        statusCode: 500,
        message: 'Неизвестная ошибка при обновлении офисного работника'
      })
    }
  }
})
