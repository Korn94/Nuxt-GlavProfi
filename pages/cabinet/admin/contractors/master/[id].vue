  ...
  <div class="container">
    ...
    <div v-else>
      ...
      <div class="block" v-if="editing">
        ...
        <input v-model="master.name" placeholder="Имя" required />
        <input v-model="master.phone" placeholder="Телефон" />
...
async function fetchMaster() {
  try {
    const data = await $fetch(`/api/contractors/masters/${route.params.id}`, {
      method: 'GET',
      credentials: 'include'
    })
    master.value = data
...
async function fetchAgreements() {
  try {
    agreements.value = await $fetch(`/api/agreements`, {
      method: 'GET',
      params: { masterId: route.params.id },
      credentials: 'include'
    })
  } catch (err) {
...
async function addWork() {
  ...
  if (!isWorkValid.value) {
    if (!newWork.value.amount) formErrors.value.amount = 'Сумма мастеру обязательна'
...
async function saveMaster() {
  try {
    await $fetch(`/api/contractors/masters/${route.params.id}`, {
      method: 'PUT',
      body: master.value,
      credentials: 'include'
    })
...
async function addAgreement() {
  ...
  try {
    const payload = {
      ...
      masterId: parseInt(route.params.id)
    }
